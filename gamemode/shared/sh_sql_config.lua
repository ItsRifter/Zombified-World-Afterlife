...
GM.MYSQL.Data.Type = "txt" --Usable types: mysql | txt (expect txt to cause issues)

if GM.MYSQL.Data.Type == "mysql" then
	GM.MYSQL.Data.Host = "127.0.0.1" --IP to the server running your myMYSQL server (use localhost to connect to a server running on the same machine)
	...
	GM.MYSQL.Data.MyMYSQLSnapshotInterval = 5 * 60
end